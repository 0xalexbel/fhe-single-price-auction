...
task(
  "transfer-payment-erc20-token",
  "Transfers payment erc20 tokens from the deployer address to a specified user"
)
  .addParam("amount", "Amount of payment erc20 tokens to transfer")
  ...
  .setAction(async function (
    ...
    const ERC20 = await deployments.get("PaymentERC20");
    const erc20: PaymentERC20 = await hre.ethers.getContractAt(
      "PaymentERC20",
      ERC20.address
    );
    ...
    console.info(
      `Transfer payment erc20 tokens from ${deployer} to: ${toAddr} amount: ${taskArguments.amount}`
    );
  });

task(
  "balance-payment-erc20-token",
  "Prints the payment erc20 token balance of a specified account"
)
