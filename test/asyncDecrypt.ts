...
function isMockWithoutSolidityCoverage(
  hre: HardhatRuntimeEnvironment
): boolean {
  return isMock(hre) && !isSolidityCoverageRunning(hre);
}

export const initGateway = async (
  hre: HardhatRuntimeEnvironment
): Promise<void> => {
  ...
  if (isMockWithoutSolidityCoverage(hre)) {
    ...
    await hre.ethers.provider.send("set_lastBlockSnapshotForDecrypt", [
      firstBlockListening,
    ]);
  }
  // this function will emit logs for every request and fulfilment of a decryption
  gateway = await hre.ethers.getContractAt(
    gatewayArtifact.abi,
    GATEWAYCONTRACT_ADDRESS
  );
  gateway.on(
    ...
    async (
      ...
      console.log(
        `${currentTime()} - Requested decrypt on block ${blockNumber} (requestID ${requestID})`
      );
    }
  );
  gateway.on(
    ...
    async (requestID, success, result, eventData) => {
      const blockNumber = eventData.log.blockNumber;
      console.log(
        `${currentTime()} - Fulfilled decrypt on block ${blockNumber} (requestID ${requestID})`
      );
    }
  );
};
...
const getAlreadyFulfilledDecryptions = async (
  ...
  const computedResults = results.concat(
    pastResults.map(
      (result) => ifaceResultCallback(hre).parseLog(result)!.args[0]
    )
  );
  //@ts-ignore
    ...
    if (!results.includes(requestID)) {
      ...
      if (mocked) {
        ...
        const isAllowedForDec = await Promise.all(
          handles.map(async (handle) => acl.isAllowedForDecryption(handle))
        );
        ...
        const types = typesList.map(
          (num) => CiphertextType[num as keyof typeof CiphertextType]
        );
        const values = await Promise.all(
          handles.map(async (handle) => BigInt(await getClearText(handle)))
        );
        ...
        const valuesFormatted2 = valuesFormatted.map((value, index) =>
          typesList[index] === 9
            ? "0x" + value.toString(16).padStart(128, "0")
            : value
        );
        const valuesFormatted3 = valuesFormatted2.map((value, index) =>
          typesList[index] === 10
            ? "0x" + value.toString(16).padStart(256, "0")
            : value
        );
        ...
        const relayer = await impersonateAddress(
          hre,
...
async function computeDecryptSignatures(
  ...
  for (let idx = 0; idx < numSigners; idx++) {
    ...
    if (privKeySigner) {
      const kmsSigner = new hre.ethers.Wallet(privKeySigner).connect(
        hre.ethers.provider
      );
      ...
      throw new Error(
        `Private key for signer ${idx} not found in environment variables`
      );
    }
  }
  return signatures;
