...
describe("TestAsyncDecrypt", function () {
  ...
  it("test async decrypt bool would fail if maxTimestamp is above 1 day", async function () {
    await expect(
      this.contract.connect(this.signers.carol).requestBoolAboveDelay()
    ).to.be.revertedWith("maxTimestamp exceeded MAX_DELAY");
  });
  ...
  it("test async decrypt uint128 non-trivial", async function () {
    const inputAlice = this.fhevm.createEncryptedInput(
      this.contractAddress,
      this.signers.alice.address
    );
    inputAlice.add128(184467440737095500429401496n);
    const encryptedAmount = await inputAlice.encrypt();
    const tx = await this.contract.requestUint128NonTrivial(
  ...
  it("test async decrypt uint256", async function () {
    ...
    expect(y).to.equal(
      27606985387162255149739023449108101809804435888681546220650096895197251n
    );
  });
  ...
  it("test async decrypt ebytes128 non-trivial", async function () {
    ...
    const encryptedAmount = await inputAlice.encrypt();
    const tx = await this.contract.requestEbytes128NonTrivial(
      encryptedAmount.handles[0],
      encryptedAmount.inputProof
    );
    await tx.wait();
  ...
  it("test async decrypt ebytes256 non-trivial with snapshot [skip-on-coverage]", async function () {
    if (network.name === "hardhat") {
      ...
      const inputAlice2 = this.fhevm.createEncryptedInput(
        this.contractAddress,
        this.signers.alice.address
      );
  ...
  it("test async decrypt mixed with ebytes256", async function () {
    ...
    const encryptedAmount = await inputAlice.encrypt();
    const tx = await this.contract.requestMixedBytes256(
      encryptedAmount.handles[0],
      encryptedAmount.inputProof
    );
    await tx.wait();
